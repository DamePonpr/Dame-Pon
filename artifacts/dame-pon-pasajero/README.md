...
```sh
pnpm --filter @workspace/dame-pon-pasajero run verify:supabase-config
```
...
```sh
pnpm --filter @workspace/dame-pon-pasajero run verify:trip-realtime
```
...
```sh
pnpm --filter @workspace/dame-pon-pasajero run verify:rls-hardening
```
